...
def weight_variable(shape):
    initial = tf.truncated_normal(shape, stddev=0.1) # A tensor of the specified shape filled with random truncated normal values. (note: default mean is 0)
    return tf.Variable(initial)

def bias_variable(shape):
    initial = tf.constant(0.1, shape=shape) # Constant of 0.1
    return tf.Variable(initial)

#   Convolution and Pooling - we do Convolution, and then pooling to control overfitting
def conv2d(x, W):
    return tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding='SAME') # Don't really get everything happening here
