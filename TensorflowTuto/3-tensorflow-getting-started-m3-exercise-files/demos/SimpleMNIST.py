...
# Tanguy: Is the reduce mean actually usefull? TF does not need the cross_entropy as a vector to do the backpropagation?
# Tanguy: is Softmax actually applied two times? 
# https://www.tensorflow.org/api_docs/python/tf/nn/softmax_cross_entropy_with_logits: Do not call this op with the output of softmax, as it will produce incorrect results.
# Tanguy: reminder: see deep learning book on cross entropy
...
# PREDICTED (y) and ACTUAL (y_). (changed by tanguy)
correct_prediction = tf.equal(tf.argmax(y,1), tf.argmax(y_,1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
test_accuracy = sess.run(accuracy, feed_dict={x: mnist.test.images, y_: mnist.test.labels})
# Tanguy: this operation is long because it requires to compute y using the trained model for each image
# I somehow thought it would be possible to use last y computed, but it is only on 100 images, so no
